...
class FintechEvaluator(BaseDomainEvaluator):
    ...
    def _detect_payment_provider(
        self, pattern_names: set, patterns: List[PatternMatch]
    ) -> str:
    ...
    async def _calculate_security_score(self, patterns: List[PatternMatch]) -> float:
        ...
        critical_patterns = [
            p for p in security_patterns if p.severity in ("critical", "high")
        ]
    ...
    async def _calculate_compliance_score(self, patterns: List[PatternMatch]) -> float:
        """
    ...
    async def _calculate_best_practices_score(
        ...
        payment_count = sum(1 for p in self.PAYMENT_PATTERNS if p in pattern_names)
    ...
    async def _identify_insights(
        ...
        if len(security_patterns) < 2:
            weaknesses.append(
                "Insufficient security patterns for financial application"
            )
        ...
        if "transaction_security" not in pattern_names:
            weaknesses.append(
                "No idempotency patterns - risk of duplicate transactions"
            )
