...
class Web3Evaluator(BaseDomainEvaluator):
    ...
    async def calculate_domain_score(
        ...
        breakdown = {
            ...
            "defi_features_count": sum(
                1 for p in patterns 
                if p.pattern_name in self.ADVANCED_DEFI_PATTERNS
            ),
            "total_sol_files": file_analysis.get("files_by_extension", {}).get(".sol", 0),
        }
    ...
    async def _calculate_innovation_score(
        ...
        advanced_count = sum(
            1 for p in self.ADVANCED_DEFI_PATTERNS 
    ...
    async def _identify_insights(
        ...
        # Weaknesses
        if "reentrancy_guard" not in pattern_names:
            weaknesses.append("Missing reentrancy protection - critical vulnerability risk")
