...
def detect_domain_from_files(repo_path: str) -> Tuple[DomainType, float]:
    ...
    logger.info(
        f"Domain detection: {detected_domain.value} "
...
def _analyze_dependencies(repo: Path) -> Dict[str, float]:
    ...
    # ML Python packages
    ml_packages = {
        'tensorflow', 'torch', 'pytorch', 'keras', 'sklearn', 
        'scikit-learn', 'transformers', 'numpy', 'pandas', 'scipy',
        'matplotlib', 'seaborn', 'mlflow', 'wandb', 'optuna'
    }
...
def calculate_weighted_score(
    ...
    return weighted_sum / total_weight * (total_weight)  # Normalize if weights don't sum to 1
