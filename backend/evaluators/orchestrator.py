...
class DomainOrchestrator:
    ...
    async def evaluate_repository(
        ...
        detected_domain, detection_confidence = await self._detect_or_use_domain(
            repo_path, domain, evaluation_id
        )
        ...
        logger.info(f"[{evaluation_id}] Running {detected_domain.value} evaluation")
        ...
        if self.config.get("evaluate_secondary_domains", False):
            secondary = get_secondary_domains(repo_path, detected_domain, threshold=0.3)
    ...
    async def evaluate_multiple_domains(
        ...
        if self.config.get("parallel_evaluation", True):
            ...
            tasks = [
                self.evaluate_repository(repo_path, domain=domain, metadata=metadata)
            ...
            for domain, result in zip(domains, completed):
                if isinstance(result, Exception):
                    logger.error(f"Evaluation failed for {domain.value}: {str(result)}")
                    continue
            ...
            for domain in domains:
                try:
                    result = await self.evaluate_repository(
                        repo_path, domain=domain, metadata=metadata
                    )
                    results[domain] = result
                except Exception as e:
    ...
    async def detect_domain(self, repo_path: str) -> Tuple[DomainType, float]:
        """
    ...
    async def _detect_or_use_domain(
        self, repo_path: str, explicit_domain: Optional[DomainType], evaluation_id: str
