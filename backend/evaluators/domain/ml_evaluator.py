...
class MLEvaluator(BaseDomainEvaluator):
    ...
    async def calculate_domain_score(
        ...
        breakdown = {
            ...
            "has_notebooks": file_analysis.get("files_by_extension", {}).get(".ipynb", 0) > 0,
        }
    ...
    async def _calculate_innovation_score(
        ...
        advanced_count = sum(
            1 for p in self.ADVANCED_ML_PATTERNS 
    ...
    async def _generate_recommendations(
        ...
        if "onnx_export" not in pattern_names and "model_quantization" not in pattern_names:
            recommendations.append(
    ...
    async def _identify_insights(
        ...
        if len(bp_patterns) >= 5:
            strengths.append("Strong ML engineering practices with multiple best patterns")
        ...
        if ipynb_files > py_files:
            weaknesses.append("Heavy reliance on notebooks - consider refactoring to modules")
